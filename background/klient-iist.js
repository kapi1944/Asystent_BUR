(function zarejestrujKlientaIist(globalny) {
  ...
  function wyciągnijŁączaIist(html) {
    ...
    wzorce.forEach(function odczytajWzorcem(wzorzec, indeksWzorca) {
      ...
      while ((trafienie = wzorzec.exec(tekst))) {
        ...
        if (!url || !/,\d+\.html(?:[?#]|$)/i.test(url)) { continue; }
  ...
  async function szukajŁączaIist(fraza) {
    ...
    try {
      const źródła = await Promise.all([adresBazowyIist, adresBazowyIist + "sitemap.xml"].map(async function pobierz(adres) {
        try {
      ...
      const kandydaci = Array.from(mapa.values()).map(function oceń(kandydat) {
      ...
      for (const kandydat of kandydaci) {
        try {
          ...
          if (przestrzeń.czyMocneDopasowanieTytułu(tytuł, czystaFraza)) { zweryfikowani.push({ url: kandydat.url, tytuł: tytuł, title: tytuł }); }
